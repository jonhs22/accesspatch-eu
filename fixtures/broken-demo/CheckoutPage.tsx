...
export function CheckoutPage() {
  ...
  function trapEmailTab(event: KeyboardEvent<HTMLInputElement>) {
    // ACCESSPATCH-DEMO-002: Keyboard focus is deliberately returned to the email input on every Tab.
    if (event.key === "Tab") {
      event.preventDefault();
              ...
              <form className="checkout-dialog" role="dialog" aria-modal="true" onSubmit={handleSubmit} aria-labelledby="checkout-title">
                ...
                <div className="email-control">
                  ...
                  <input
                    ...
                    onKeyDown={trapEmailTab}
                    aria-invalid={error}
                  />
                </div>
                {/* ACCESSPATCH-DEMO-001: This icon-only payment control has no accessible name. */}
                <button className="payment-submit" data-testid="payment-submit" type="submit">
                  <CardIcon />
                </button>
                {/* ACCESSPATCH-DEMO-003: The visible validation text is not announced to assistive technology. */}
                {error && <p className="form-error" data-testid="form-error">{EMAIL_ERROR}</p>}
              </form>
            )}
